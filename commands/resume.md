...
- **이미 완료된 필드를 사용자가 먼저 자발적으로 정정하려 하면**(예: 답변 도중 "아, 자본은 사실 …야"):
  이 명령에서 즉시 반영하지 말고 "그건 `/business-counselor:edit`로 바꾸실 수 있어요" 안내 후 원래
  누락 필드 질문 계속 진행 (resume=빈 곳 채우기, edit=기존 값 수정으로 역할 분리 유지, v0.5.0)
