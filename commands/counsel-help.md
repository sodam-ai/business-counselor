...
[추천 흐름]
1) /counsel:start                 나에 대한 인터뷰 (처음 한 번) → profile.md 생성
2) /counsel:evaluate "아이디어"   아이디어 5단계 냉철 판독 → 결과 저장
3) /counsel:list                  지금까지 판독 목록 보기
4) /counsel:show <id>             특정 판독 다시 보기
   /counsel:resume                인터뷰 이어서(정보 보완)

[명령 5개]
- /counsel:start                  자기 인터뷰 시작 (Mom Test 6분야, 약 30~40분)
- /counsel:resume                 빠진 항목만 추가 질문해 프로필 보완
- /counsel:evaluate "<아이디어>"  13관점·Lean Canvas·Mom Test·Pre-mortem·적대토론 단일 판독
- /counsel:list                   판독 기록 목록 (id·판정·확신도)
- /counsel:show <id>              해당 판독 전체 다시 보기
