...
- **`raw_idea`에 큰따옴표(`"`)·역슬래시(`\`)·줄바꿈이 포함되면 반드시 YAML 표준 이스케이프 처리 후 기록**
  (`"` → `\"`, `\` → `\\`, 줄바꿈 → 공백 또는 `\n`) — 그대로 삽입하면 frontmatter YAML 파싱이 깨져
  해당 평가 파일 전체를 읽을 수 없게 될 수 있음(2026-08-03 경계값 검토로 발견. `commands/decide.md`의
  `note` 필드 JSON 이스케이프 누락과 동일한 유형의 결함)
