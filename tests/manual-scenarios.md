...
### 절차
1. Claude Code에서 `/counsel:start` 입력
...
### 절차
1. `/counsel:evaluate "AI로 뭔가 사업 해보고 싶어"` 입력 (모호 입력)
...
### 절차
1. `/counsel:evaluate "AI로 한국 부동산 매물 자동 분석 + 투자 가치 점수화 SaaS"` 입력
...
2. `/counsel:start` 1회 + `/counsel:evaluate` 1회 + `/counsel:list` 1회 실행
3. 환경 스냅샷 2차 (동일 명령들)
4. 1차 vs 2차 비교
...
- [ ] 슬래시 명령 충돌 0 — `/counsel:*` 5개가 다른 플러그인 명령과 겹치지 않음
...
- 명령·에이전트 충돌 → prefix 강화 (`/counsel:` → `/business-counselor:` 검토)
...
- [ ] 5건 누적 시 외부 API 호출 0 (Phase 1 정책)
- [ ] 5건 누적 시 별도 서브에이전트 호출 0 (단일 호출 정책)
- [ ] 5건 누적 후 사용자 환경 무결성 확인 (시나리오 5와 동일 검사 재실행)
...
위 5건 그대로 재실행 + 다음 추가:
- [ ] `/counsel:recommend 5` 정상 동작
- [ ] `/counsel:decide <id> go` decisions.jsonl 1줄 추가
...
위 + Phase 2 추가 + 다음:
- [ ] `/counsel:research <topic>` 명시 호출 시만 외부 API 활성
