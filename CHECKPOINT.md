...
프로세스는 옛 정의를 메모리에 들고 있음).

**3차 조치(같은 날, v0.6.3 병합 직후)**: PR #24(raw_idea/title YAML 이스케이프 수정) 병합 후 marketplace
클론을 `v0.6.3`(커밋 `79c7d71`)로 재차 pull. 이번엔 캐시 폴더가 자동 생성되지 않은 상태였으므로, 소스
클론 내용을 `.git` 제외하고 그대로 복사해 `plugins/cache/.../business-counselor/0.6.3/` 캐시 폴더를 직접
구성 → 파일 목록·`plugin.json` 내용이 소스와 100% 일치함을 diff로 재확인 → `installed_plugins.json` 재백업
