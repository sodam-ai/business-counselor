...
**설치 후 Claude Code를 재시작**하면 `/counsel:` 명령을 쓸 수 있습니다.
...
```
/counsel:evaluate "AI로 부동산 매물 분석해주는 서비스"
```
...
| 명령어 | 설명 |
|--------|------|
| `/counsel:help` | 사용법·명령·용어를 한 화면에 요약 |
| `/counsel:start` | 나에 대한 AI 인터뷰 시작 (처음 한 번) |
| `/counsel:resume` | 인터뷰 이어서 하기 (빠진 정보 보완) |
| `/counsel:evaluate "아이디어"` | 아이디어 분석 — 기본은 한눈 요약 카드 |
| `/counsel:evaluate "아이디어" 전체` | 아이디어 분석 — §1~§5 전체 상세 |
| `/counsel:list` | 지금까지 분석한 목록 (id·판정·확신도) |
| `/counsel:show <id>` | 특정 분석 전체 다시 보기 |

> 막히면 언제든 `/counsel:help` 를 먼저 입력하세요.
...
```
[처음 1회]  /counsel:start  →  인터뷰 답변  →  내 프로필(profile.md) 저장
                                   │
[매번]  /counsel:evaluate "아이디어"
            │
        ...
        저장: 결과 파일(.md)  →  /counsel:list 로 목록, /counsel:show 로 다시 보기
```
...
| 결과가 카드만 짧게 나옴 | 기본이 카드 모드 | 전체는 `/counsel:evaluate "..." 전체` 또는 `/counsel:show <id>` |
| 다른 플러그인의 `uv: command not found` 등 오류 | **이 플러그인과 무관** | 무시해도 됩니다 (다른 도구의 알림) |
| 프로필 없이 평가됨 | 인터뷰 미진행 | 일반 기준으로 평가됩니다. 맞춤 원하면 `/counsel:start` 먼저 |

> 그래도 안 되면 `/counsel:help` 를 입력해 현재 사용법을 확인하세요.
...
- 아이디어 자동 생성(`/counsel:recommend`) 미지원 → Phase 2 예정
