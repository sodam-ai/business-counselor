...
- **환경변수**: 이 플러그인은 별도의 환경변수를 요구하지 않습니다. API 키·비밀번호 등을 직접 설정할 필요가 없으며, Claude Code 자체 로그인 정보 외에는 아무것도 입력하지 않습니다.
...
### 17.0 쉬운 말 요약 (자세한 내용은 아래 17.1~17.6)

- ✅ **써도 되는 것**: 코드를 그대로 쓰거나 고쳐서 내 제품에 넣기, 복제·포크, 재배포, 유료로 팔거나 서비스로 운영하기, 이 도구가 만든 분석 결과를 사업계획서·강의자료·고객사 납품물에 쓰기 — 전부 **가능**합니다(Apache-2.0 조건만 지키면 됨).
- 🔍 **직접 확인해야 하는 것**: 이 도구가 만든 분석·추천 결과가 실제로 맞는지, 다른 회사·상품과 이름이 겹치지 않는지, Claude Code 자체의 요금·약관은 어떻게 되는지 — 이 프로젝트가 대신 보장해주지 않으므로 **사용자가 직접** 확인해야 합니다.
- ⛔ **하면 안 되는 것(또는 위험한 것)**: 이 도구의 분석을 실제 투자 자문·법률 자문·세무 자문인 것처럼 제공하는 것, "SoDam AI Studio"·"business-counselor" 이름을 마치 공식 제휴·보증인 것처럼 쓰는 것.
...
- **NOTICE 파일**: 이 저장소에는 별도 `NOTICE` 파일이 **없습니다**. Apache-2.0 제4조(d)는 원본 저작물에
  NOTICE 파일이 포함된 경우에만 이를 함께 배포하도록 요구하는데, 이 프로젝트는 다른 Apache-2.0 프로젝트를
  포크·수정한 것이 아니라 처음부터 새로 작성된 저작물이라 해당 조건이 적용되지 않는다고 판단했습니다
  (2026-08-04 점검). *다만 이 판단 자체가 법적 결론은 아니므로, 재배포·상업적 활용 전 NOTICE 필요 여부를
  스스로도 한 번 더 확인하시거나 법무 검토를 권장합니다 — [법무/전문가 검토 필요].*
- **보장하지 않는 것**:
...
- **AI 생성 콘텐츠 확인 의무**: 이 도구의 결과물(§1~§5 분석, Lean Canvas, 추천 아이디어의 제목·설명 등)은
  AI가 생성한 콘텐츠입니다. 사업계획서·투자유치 자료·고객사 제출 문서 등 **실제로 외부에 내보내거나
  상업적으로 활용하기 전에**, 다음을 사용자가 직접 확인하시기 바랍니다: (1) 특정 회사·제품·상표명이
  실수로 포함되지는 않았는지, (2) 이미 존재하는 사업 모델·브랜드·저작물과 지나치게 유사하지는 않은지,
  (3) 통계·수치의 출처가 실제로 신뢰할 만한지. 이 프로젝트는 AI 생성 결과물이 제3자의 저작권·상표권을
  침해하지 않는다고 보장하지 않습니다. [법무/전문가 검토 필요 — AI 생성물의 저작권 귀속·침해 판단은
  관할 법령·최신 판례에 따라 다를 수 있음]
...
| 코드 복제·포크(GitHub Fork 포함) | 가능 | Apache-2.0이 명시적으로 허용. 포크 후 저장소 이름·설명에서 원저작물과 혼동되지 않게 하는 것을 권장 |
| 이 도구로 만든 사업 분석 결과를 사업계획서·투자유치 자료에 활용 | 가능 | 정확성 보장 없음 — 별도 검증 필수(§17.2 "AI 생성 콘텐츠 확인 의무" 참고) |
| 이 도구를 재판매(유료 서비스화)·SaaS로 서비스 운영 | 가능 | Apache-2.0은 재판매·서비스 운영을 금지하지 않음. 단, "SoDam AI Studio"·"business-counselor" 이름/상표를 마치 공식 제휴·보증인 것처럼 사용하는 것은 권장하지 않음 |
| 이 도구(코드 또는 결과물)를 교육 자료·강의 콘텐츠로 활용 | 가능 | Apache-2.0 조건(라이선스 고지) 준수. 결과물을 교재 사례로 쓸 경우 AI 생성물임을 명시하는 것을 권장 |
| 이 도구로 만든 분석 결과를 회사/고객사에 납품·인도 | 가능 | 결과물 자체의 소유권은 사용자에게 있음(§17.2). 다만 정확성·완전성을 보증하지 않으므로, 납품 전 별도 검증 및 고객사에 "AI 생성·참고용" 사실을 고지하는 것을 권장 |
| 이 도구의 분석을 실제 투자 자문·법률 자문으로 제공 | **권장하지 않음 / 위험** | §17.3 참고 — 별도의 인허가·자격이 필요한 영역일 수 있음 |

### 17.6 별도로 확인해야 하는 외부 서비스 약관

이 플러그인 자체는 요금을 청구하지 않지만(§2 참고), **Claude Code·Claude 모델 사용 자체는 Anthropic의
자체 이용약관·사용 정책·요금 정책을 따릅니다.** 이 프로젝트(SoDam AI Studio)는 Anthropic의 약관·정책을
대신 보장하거나 요약하지 않으며, 다음은 사용자가 Anthropic 공식 채널(Claude Code 설치·로그인 과정에
안내되는 약관, [anthropic.com](https://www.anthropic.com))에서 **직접 확인해야 합니다**:

- Claude Code/Claude 계정의 요금제·사용량 한도
- Claude 모델의 사용 정책(Usage Policy) — 특히 이 도구의 결과물을 상업적으로 재배포·재판매할 때 모델
  출력물에 대한 Anthropic 측 조건이 있는지
- 데이터 보관·학습 활용 정책(사용자가 Claude Code에 입력하는 내용 전반에 적용되는 Anthropic 자체 정책이며,
  이 플러그인이 별도로 통제하지 않음)

이 플러그인은 이미지·폰트·아이콘·영상·음원 등 별도의 외부 미디어 자산을 포함하지 않습니다(2026-08-04
저장소 전수 확인 — `assets`/`public`/이미지·폰트 확장자 파일 0건). 따라서 별도의 폰트·이미지 라이선스
확인은 현재 버전 기준으로는 필요하지 않습니다. 향후 이미지·폰트 등을 추가하게 되면 그 시점에 각 자산의
라이선스를 별도로 점검해야 합니다.
