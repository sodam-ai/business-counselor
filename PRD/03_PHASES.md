...
### 가드레일 (필수 준수)
- 모든 슬래시 명령은 `/business-counselor:*` 네임스페이스 강제
...
- [ ] 출력 분량: 깊이 우선 (브레비티는 「한눈 요약」 카드, `<6,000`은 가이드 — v0.2.0)
...
- 모든 명령 /business-counselor:* 네임스페이스
- 모든 서브에이전트 bc- prefix
...
### 완료 기준 (v1.1 정량화)
- 사용자가 `/business-counselor:start`로 인터뷰 → profile.md 작성 완료 (인터뷰 30~40분 범위)
- 사용자가 임의 아이디어로 `/business-counselor:evaluate` 실행 → **5단계 결과(Pre-mortem 3개·Bull/Bear/Judge 3섹션 포함)** + 면책 + success_criteria + consistency_score(placeholder) frontmatter 출력
- `tests/manual-scenarios.md`의 **5건 시나리오 모두 PASS** (시나리오 1~5)
- 기존 환경(bkit, everything-claude-code 등)에 영향 0 확인 (시나리오 5)
- **API 호출 최소화 정책 준수 검증** — 한 `/business-counselor:evaluate` 호출 시 추가 서브에이전트 호출 0건
...
### 기능
- [ ] 명령 `/business-counselor:research <topic>` — deep-research 서브에이전트 호출
- [ ] 명령 `/business-counselor:followup <eval-id>` — 과거 평가 재방문 + 시장 변화 비교
- [ ] 명령 `/business-counselor:stats` — go/drop 비율·명중률·도메인 패턴 요약
...
- **default 비활성** — 사용자가 `/business-counselor:research` 명시 입력 시만 활성. 자동 백그라운드 호출 0.
...
- **월 호출 한도: 100회로 확정 (2026-08-02)** — PRD 제안값을 그대로 채택.
  근거: 자동 백그라운드 호출이 0이라(사용자 명시 호출만) 하루 평균 3.3회 수준이면 도달, 일반 개인 사용
  패턴에서는 여유 있는 상한. 카운트는 `external_calls.jsonl`의 이번 달 타임스탬프 개수 집계로 계산(별도
  DB 불필요). 한도 도달 시: **하드 차단**(추가 호출 자동 진행 금지) + "이번 달 외부 검색 한도(100회)를
  모두 사용했습니다. 다음 달 1일 초기화됩니다" 안내. 수동 상향은 지원하지 않음(v1 범위 밖 — 필요성 확인되면
  후속 버전에서 사용자 설정 가능하게 확장)
