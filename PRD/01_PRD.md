...
**시나리오 2 — 아이디어 가져오기 (판독)**
1. 사용자가 `/business-counselor:evaluate "AI로 한국 부동산 매물 자동 분석"` 입력
...
1. 사용자가 `/business-counselor:recommend 5` 입력
2. AI가 누적 프로필 기반 사업 아이디어 5개 + 각 Lean Canvas 출력
3. 사용자가 마음에 드는 1~2개에 대해 `/business-counselor:evaluate <id>`로 심층 판독

**시나리오 4 — 재방문 (Phase 3)**
1. 3개월 뒤 `/business-counselor:followup eval-001` 입력
...
| B. AI 사업 아이디어 추천 (`/business-counselor:recommend`) | 누적 프로필 기반 N개 아이디어 + 각 Lean Canvas | P2 | 보통 |
| 결정 로그 (`/business-counselor:list`, `/business-counselor:show`) | 과거 추천·판독 검색·재참조 | P2 | 간단 |
| Pre-mortem 프레임 정교화 | "이 사업이 망한다면 왜?" 시나리오 5개 자동 생성 (Phase 1은 3개 OK) | P2 | 보통 |
| D. 외부 시장 리서치 (`/business-counselor:research`) | deep-research 또는 WebSearch 호출. **사용자 명시 호출 시만 활성** (default 비활성) | P3 | 복잡 |
| 재방문 (`/business-counselor:followup`) | 과거 아이디어 시장 변화 반영 재평가 | P3 | 보통 |
| 결정 통계 (`/business-counselor:stats`) | go/drop 비율·명중률 패턴 요약 | P3 | 간단 |
...
### 핵심 흐름 — 인터뷰 → 판독
```
/business-counselor:start → AI 질문 → 사용자 답 → profile.md 누적 → /business-counselor:evaluate <아이디어>
   → 4프레임 분석 → 점수·위험·go/no-go → evaluated/*.md 저장 → 면책 조항 출력
```

### 상세 흐름 (Phase 1 기준)

1. **인터뷰 시작 (`/business-counselor:start`)**: 처음이면 빈 profile.md 생성. AI가 Mom Test 스타일 6개 카테고리(생애사·자본·시간·역량·관심도메인·리스크 성향) 질문 시작
2. **누적 (`/business-counselor:resume`)**: 다음 세션에서 이어서. 부족 영역 자동 감지하여 우선 질문
3. **아이디어 입력 (`/business-counselor:evaluate "<idea>"`)**: 한 줄 또는 paragraph 입력. 모호하면 AI가 보강 질문 1~2개
...
6. **결정 기록**: 사용자가 선택 시 `/business-counselor:decide <id> <go|drop|iterate>` → decisions.jsonl 1줄 추가
...
- [ ] `/business-counselor:start`로 인터뷰 시작·답변·profile.md 생성·세션 종료 후 재시작 정상 동작 (인터뷰 30~40분 범위)
...
### Phase 2 추가 기준
- [ ] `/business-counselor:recommend 5` → 5개 아이디어 + 각 Lean Canvas 정상 출력
...
- [ ] `/business-counselor:followup` 시 과거 판독과 변경점 명확 표시
...
- **default 활성 외부 API 호출** — 이유: API 절약 정책. Phase 3 외부 호출은 사용자가 `/business-counselor:research` 명시 입력 시만 활성. 자동 백그라운드 호출 0
...
- ✓ ~~**적대 토론 시스템 프롬프트 최종 문구**~~ → **v1.2 본문 박힘** (04_PROJECT_SPEC.md §5단계 적용 순서, Bull/Bear/Judge 각 1단락 완성. 2026-07-27 문서 간 불일치 발견·동기화)
- ✓ ~~**success_criteria 측정 지표 표준 세트**~~ → **v1.2 카탈로그 5개 완성** (04_PROJECT_SPEC.md §"항상 해", ①고객인터뷰~⑤시장리서치. 2026-07-27 문서 간 불일치 발견·동기화)
